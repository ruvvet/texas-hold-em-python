...
# Input validation to make sure they arent betting more $
# than they have available
    ...
    def player_state(self, check):
        ...
        # they have folded and cannot participate this match
        if not self.player_cards:
            ...
            # If player raises, everyone else mutch match or re-raise
                ...
                print(
                    'Raise the current high bet (${bet}) for this round.'
                    '(Your bets this betting round/Total chips on the table:'
                    '${current_bet}/${total_bet}).'.format(
                ...
                self.bet_amt = (
                    self.input_money(RAISE_AMT) + self.community.match_raise
                )

                # Set so that 'Raise' always is added to the current high bid
                # Circumvents the issue of needing a check

                self.community.match_raise = self.bet_amt
                print(
                    PLAYER_RAISE + '{raise_amt}.'
                    'High bid is now: ${bet}.'.format(
    ...
    def full_hand(self):
        self.hand_str = (
            self.cards.get_hand(self.player_cards, self.community.comm_cards)
            )
    ...
    def player_summary(self, player, bet_round):
        print(
            ...
            '\n >> Player History: {player_history}'.format(
                ...
                player_history=(
                    self.history[bet_round]
                    if bet_round in self.history else [])
                )
            )
