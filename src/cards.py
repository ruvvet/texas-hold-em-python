...
class Cards:
    ...
    def __init__(self, hand):
        ...
        self.RANKED_HAND_TYPES = dict(zip(
            [x for x in range(10)], self.HAND_TYPES
            ))
    ...
    def get_hand(self, player_cards, community_cards):
        ...
        self.values = [
            i[0] for i in self.hand if i[0] in self.CARD_VALUES.keys()
            ]
        # Gives all the suits in the hand
        self.suits = [
            j[1] for j in self.hand if j[1] in self.CARD_SUITS.keys()
            ]
        # Sorts all values in hand in desc order
        self.sort_hand = sorted(
            [self.CARD_VALUES[i] for i in self.values], reverse=True
            )
    ...
    def evaluate_hand(self):
        ...
        self.best_hand = sorted(
            [k for k, v in self.RANKED_HANDS.items() if v[0]], reverse=True
            )[0]
        self.top_card = [
            v[1] for k, v in self.RANKED_HANDS.items() if k == self.best_hand
            ]
    ...
    def pair(self):
        if 2 in self.COUNT.values():
            pair = sorted(
                [self.CARD_VALUES[k] for k, v in self.COUNT.items() if v == 2],
                reverse=True
                )
    ...
    def three_kind(self):
        if 3 in self.COUNT.values():
            three = sorted(
                [self.CARD_VALUES[k] for k, v in self.COUNT.items() if v == 3],
                reverse=True
                )
    ...
    def straight(self):
        return ''.join(map(str, self.sort_hand)) in '14131211109876543214',
        self.sort_hand[0]
    ...
    def four_kind(self):
        if 4 in self.COUNT.values():
            four = sorted(
                [self.CARD_VALUES[key] for k, v in self.COUNT.items()
                    if v == 4],
                reverse=True
                )
    ...
    def royal_flush(self):
        return self.flush()[0] \
            and sorted(self.sort_hand) == list(range(10, 15)),
        self.sort_hand[0]
    ...
    def prettify(self, card):
        self.card = card
        return "[", str(self.CARD_VALUES[self.card[0]]), \
            self.CARD_SUITS[self.card[1]], "]"
