...
class Board:
    ...
    def rounds(self):
        ...
        self.bet_round = [
            'Pre-flop Betting', '2nd Betting', '3rd Betting', '4th Betting'
            ]
        ...
        while self.round_counter < 4:
            ...
            print('\n ⌛ BETTING ROUND: {bet_round}'.format(
                bet_round=self.bet_round[self.round_counter].upper()
                ))
            ...
            # Draw n cards by looking up the values in the draw_round dict
            # Print community cards
            print('\n ⚡ DEAL: {deal_round}'.format(
                deal_round=self.draw_key[self.round_counter].upper()
                ))
            self.community.community_hand(self.draw_round.get(
                self.draw_key[self.round_counter]
                ))
            print('COMMUNITY CARDS: {uni_cards} {pretty_cards}'.format(
                uni_cards=self.community.cards.print_cards2(
                    self.community.comm_cards
                    ),
                pretty_cards=self.community.cards.print_cards(
                    self.community.comm_cards
                    )
                )
            )
    ...
    def start_betting(self, betting_round):
        ...
        # Call player_state function
        # and check their call to update check state

        # is there a way to remove this section
        # and have it all under the while loop?
        # When i try it gives a no attribute error
        # since it seems to try to call the player calls
        # when they dont yet exist?

        for player_num, player in self.players.items():
            # Calls player summary
            player.player_summary(player_num, betting_round)
            # print('Hand Strength: ', player.hand_strength())
            # Call player_state func,
            # then pass the player_state into check function
            # to update check state
            # player.player_state(self.community.can_check())

    # round 2 where i pass it 3x to get player history
            player.player_history(
                player.player_state(self.community.can_check()), betting_round
                )

        # Every player must either fold or
        # match the highest bet in order to move to the next round
        # If all player bet_amts are not equal to the highest raise
        # Continue prompting input
        while list(set(
            [player.bet_amt for player_num, player
                in self.players.items()
                if player.player_cards != []]
                ))[0] != self.community.match_raise:

            # print(list(set([player.bet_amt
            # for player_num, player in self.players.items()
            # if player.player_call != 'F']))[0])
            print(
                'All remaining player bets must match the current high bet.'
                '\n Amount to match: ${match}'.format(
                    match=self.community.match_raise
                    ),
                '\n [C]all, [R]aise, or [F]old'
                )

            for player_num, player in self.players.items():
                if player.player_call != 'F' \
                    and player.bet_amt < self.community.match_raise:
                    player.player_summary(player_num, betting_round)
                    # player.player_state(self.community.can_check())
                    player.player_history(
                        player.player_state(
                            self.community.can_check()
                            ), betting_round
                        )

        # Update everyone's funds here outside of the loop
        # once betting is all complete
    ...
    def check_for_winner(self, round_counter):
        # print(len([player.player_call for player_num, player
        # in self.players.items() if player.player_call == 'F']))
        ...
        # self.still_playing = [player_num for player_num, player
        # in self.players.items() if player.player_call != 'F']
        if len(self.WINNER) == 1:
            self.round_winner = True
            print('\n' + WIN_BY_FOLD)
            # self.WINNER = self.still_playing
        # i thought maybe it wasn't working since there weren't enough cards
        # and it was giving a list index out of range error
        if round_counter == 3:
            # What happens when somebody folds?
            self.round_winner = True
            self.FINAL_HANDS = {
                player_num: player.full_hand() for player_num, player
                in self.players.items()
                }
            self.WINNER = self.evaluate_winning_hand(self.FINAL_HANDS, 0)
            # self.player_round_winner =
            # [k for k,v in self.WINNER_HAND.items()]
    ...
    # Replaces old dictionary only with hands
    # who have the greatest value at that index
    ...
    def evaluate_winning_hand(self, FINAL_HANDS, counter):
        if len(FINAL_HANDS.keys()) == 1 or counter > 7:
            ...
            FINAL_HANDS = {
                k: v for k, v in FINAL_HANDS.items()
                if v[counter] >= max(self.zipped[counter])
                }
    ...
    # 1. Prints all the players who are still left in the 'final winners'
    # dictionary if there are more than 2
    ...
    def declare_winner(self, WINNER):
        if self.round_winner:
            if len(WINNER) > 1:
                print(
                    '\n Tie between players', ' & '.join(
                        ['{}']*len(WINNER.keys())).format(*WINNER.keys()
                        )
                    )
                ...
                for player_num, players in self.players.items():
                    for x in WINNER.values():
                        ...
                        self.winning_hand = [
                            players.cards.RANKED_HAND_TYPES[x[0]]
                            for x in WINNER.values()
                            ][0]
                        self.winning_card = [
                            players.cards.REVERSE_CARD_VALUES[x[1]]
                            for x in WINNER.values()
                            ][0]
                print('Winning hand: {hand_type} with {card}.'.format(
                    hand_type=self.winning_hand,
                    card=self.winning_card
                    ))
                # update player funds with whole pot
                self.players[
                    list(self.WINNER.keys())[0]
                    ].funds += self.community.pot
                # empty pot
